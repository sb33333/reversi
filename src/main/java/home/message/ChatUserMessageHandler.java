...
public class ChatUserMessageHandler implements UserMessageHandler {
    ...
    public void handleMessage(WebSocketSession session, UserMessagePayload userMessagePayload) throws Exception {
        ...
        if(reversiSessionId == null) {
            res=new ResponseMessage.Builder(MessageType.CHAT, ResponseStatus.ERROR)
        ...
        if(reversiSession.isPresent()) {
            res = new ResponseMessage.Builder(MessageType.CHAT, ResponseStatus.OK)
                .response(userMessagePayload.getText())
                .build();
            WebSocketMessage<String> chatMessage = new TextMessage(objectMapper.writeValueAsString(res));
            Set<WebSocketSession> messageTargets = reversiSession.get().usersStream()
                ...
                for (WebSocketSession messageTarget:messageTargets) {
                    messageTarget.sendMessage(chatMessage);
                }
            return;
        } else {
            res = new ResponseMessage
                .Builder(MessageType.CHAT, ResponseStatus.ERROR)
